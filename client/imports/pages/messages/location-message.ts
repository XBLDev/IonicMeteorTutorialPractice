...
export class NewLocationMessageComponent implements OnInit, OnDestroy {
  ...
  //geoloc: Geolocation;
  geolocvar : Geolocation;

  constructor(private platform: Platform, private viewCtrl: ViewController) {
  //constructor(private platform: Platform, private viewCtrl: ViewController, private geolocation: Geolocation) {
  ...
  reloadLocation() {
	return Observable.fromPromise(this.geolocvar.getCurrentPosition().then((position) => {
